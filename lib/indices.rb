INDICES = {
  de: [
    'https://www.finanzen.net/index/dax/40-werte',
    'https://www.finanzen.net/index/tecdax/werte',
    'https://www.finanzen.net/index/mdax/werte',
    'https://www.finanzen.net/index/sdax/werte',
    'https://www.finanzen.net/index/qix-deutschland/werte',
    'https://www.finanzen.net/index/vdax_new/werte',
  ],
  us: [
    'https://www.finanzen.net/index/s&p_500/werte',
    'https://www.finanzen.net/index/s&p_600_small_cap/werte',
    'https://www.finanzen.net/index/dow_jones/werte',
    'https://www.finanzen.net/index/nasdaq_100/werte'
  ],
  europe: [
    'https://www.finanzen.net/index/ftse_100/werte',
    'https://www.finanzen.net/index/cac_40/werte',
    'https://www.finanzen.net/index/ibex_35/werte',
    'https://www.finanzen.net/index/smi/werte',
    'https://www.finanzen.net/index/euro_stoxx_50/werte'
  ],
  asia:
  [
    'https://www.finanzen.net/index/nikkei_225/werte',
    'https://www.finanzen.net/index/topix-500/werte',
    'https://www.finanzen.net/index/hang_seng/werte',
  ],
  australia:
  [
    'https://www.finanzen.net/index/asx/werte'
  ]
}.freeze
